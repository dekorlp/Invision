...
private:
	...
	void Init()
	{
		...
		unsigned char* pixels = readPNG(std::string(INVISION_BASE_DIR).append("/src/tools/PushConstantDemo/Textures/viking_room.png"), width, height, channels);

		std::vector<Invision::Vector3> positions;
		std::vector<Invision::Vector2> texCoords;

		LoadModel(std::string(INVISION_BASE_DIR).append("/src/tools/PushConstantDemo/Models/viking_room.obj"), vertices, indices);
