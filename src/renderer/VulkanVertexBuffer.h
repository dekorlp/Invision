...
namespace Invision
{
	...
	public:
		...
		INVISION_API VulkanBindingDescription(VulkanInstance* instance, VulkanBaseVertexBuffer &baseVertexBuffer, uint64_t size, const void *source, uint64_t offset, uint32_t stride, VertexInputRate vertexInputRate);
	...
	public:
		...
		std::shared_ptr < IBindingDescription> CreateVertexBinding(uint64_t size, const void *source, uint64_t offset, uint32_t stride, VertexInputRate vertexInputRate);
