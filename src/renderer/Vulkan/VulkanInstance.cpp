...
namespace Invision
{
	...
	VulkanInstance::VulkanInstance() : mInstance(nullptr)
	{
#ifdef NDEBUG
	...
	SVulkan VulkanInstance::Init(const char* appName, const char* engineName, const uint32_t appVersion, const uint32_t engineVersion, const std::vector<const char*> &extensions)
	{
	...
	void VulkanInstance::SetDebugMessanger(void(*debugFunc)(VkDebugUtilsMessageSeverityFlagBitsEXT messageSeverity,
		VkDebugUtilsMessageTypeFlagsEXT messageType,
		const VkDebugUtilsMessengerCallbackDataEXT* pCallbackData))
	{
	...
	void VulkanInstance::CreateInstance(const char* appName, const char* engineName, const uint32_t appVersion, const uint32_t engineVersion, const std::vector<const char*> &extensions)
	{
...
#error Only Win32 is currently supported. To see how to support other windowing systems, \
 ...
 source code. XXX specifies the windowing system (e.g. x11 for X11, and wl for Wayland).
	...
	void VulkanInstance::CheckExtensions(std::vector<const char*> &requiredExtensions)
	{
