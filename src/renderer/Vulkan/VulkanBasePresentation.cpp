...
required to support other windowing systems.
	...
	VkImageView VulkanBasePresentation::CreateImageView(SVulkanBase &vulkanInstance, VkImage image, VkFormat format)
	{
	...
	void VulkanBasePresentation::CreateImageViews(SVulkanBase &vulkanInstance, SVulkanContext &vulkanContext)
	{
		...
		for (uint32_t i = 0; i < vulkanContext.swapChainImages.size(); i++) {
			vulkanContext.swapChainImageViews[i] = CreateImageView(vulkanInstance, vulkanContext.swapChainImages[i], vulkanContext.swapChainImageFormat);
		}
