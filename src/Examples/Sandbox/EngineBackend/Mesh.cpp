...
Mesh::Mesh(std::vector<IVertex*> vertices)
{
	mVertices = vertices;
	if (vertices.size() > 0)
	{
...
Mesh::Mesh(std::vector<IVertex*> vertices, std::vector<uint32_t> indices)
{
	mVertices = vertices;
	mIndices = indices;
	if (vertices.size() > 0)
...
{
	mVertices.push_back(new Vertex2D(vertex));
	mVertexType = vertex.getVertexType();
	mHasIndixBuffer = false;
}
...
Mesh::Mesh(std::vector<Vertex2D> vertices)
{
	for (int i = 0; i < vertices.size(); i++)
	{
		mVertices.push_back(new Vertex2D(vertices[i]));
	}
...
Mesh::Mesh(std::vector<Vertex2D> vertices, std::vector<uint32_t> indices)
{
	for (int i = 0; i < vertices.size(); i++)
	{
		mVertices.push_back(new Vertex2D(vertices[i]));
...
{
	mVertices.push_back(new Vertex3D(vertex));
	mVertexType = vertex.getVertexType();
	mHasIndixBuffer = false;
}
...
Mesh::Mesh(std::vector<Vertex3D> vertices)
{
	for (int i = 0; i < vertices.size(); i++)
	{
		mVertices.push_back(new Vertex3D(vertices[i]));
	}
...
std::vector<IVertex*> Mesh::GetVertices()
{
	return mVertices;
}
