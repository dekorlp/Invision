...
private:
	...
	void Init()
	{
		...
		unsigned char* pixels = readPNG(std::string(INVISION_BASE_DIR).append("/src/Examples/PushConstantDemo/Textures/viking_room.png"), width, height, channels);

		std::vector<Invision::Vector3> positions;
		std::vector<Invision::Vector2> texCoords;

		LoadModel(std::string(INVISION_BASE_DIR).append("/src/Examples/PushConstantDemo/Models/viking_room.obj"), vertices, indices);
		...
		auto vertShaderCode = readFile(std::string(INVISION_BASE_DIR).append("/src/Examples/PushConstantDemo/Shader/PushConstantDemo/vert.spv"));
		auto fragShaderCode = readFile(std::string(INVISION_BASE_DIR).append("/src/Examples/PushConstantDemo/Shader/PushConstantDemo/frag.spv"));
